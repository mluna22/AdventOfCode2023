...
class Hand:
  ...
  def get_type(self):
    max_same_label = 0
    max_same_label_card = 'J'
    pair_found = False
    two_pair = False
    found_cards = []

    for i in range(0, len(self.hand_)):
      if max_same_label_card == 'J' and self.hand_[i] != 'J':
        max_same_label_card = self.hand_[i]
      ...
      for j in range(i+1, len(self.hand_)):
        if self.hand_[i] == self.hand_[j]:
          ...
          if same_label >= max_same_label:
            ...
            if self.hand_[j] != 'J':
              max_same_label_card = self.hand_[j]
    ...
    if 'J' in self.hand_:
      new_hand = self.hand_.replace('J', max_same_label_card)
      max_same_label = 0
      pair_found = False
      two_pair = False
      found_cards = []
      for i in range(0, len(new_hand)):
        if new_hand[i] in found_cards:
          continue
        same_label = 0
        for j in range(i+1, len(new_hand)):
          if new_hand[i] == new_hand[j]:
            same_label += 1
            if new_hand[i] not in found_cards:
              found_cards.append(new_hand[i])
        if same_label > max_same_label:
          max_same_label = same_label
...
for i in range(len(hands)):
  print(hands[i], hands[i].bid_, i+1)
